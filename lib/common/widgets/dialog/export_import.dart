...
import 'package:PiliPlus/utils/ohos/ohos_file_picker.dart';
...
Future<void> importFromClipBoard<T>(
  ...
  if (Platform.isOhos) {
    // OHOS denies programmatic pasteboard reads without READ_PASTEBOARD
    // (a system_basic permission a normal app can't be granted). Fall back to
    // the input dialog where the user pastes via the IME (which is allowed).
    final text = await OhosFilePicker.readClipboard();
    ...
    clipboardText = (await Clipboard.getData('text/plain'))?.text;
  }
  if (clipboardText case final text? when (text.isNotEmpty)) {
    if (!context.mounted) return;
    final T json;
    ...
    if (executeImport ?? false) {
      try {
        ...
        debugPrint('PILI_IMPORT_ERR clipboard: $e\n$s');
...
Future<void> importFromLocalFile<T>({
  ...
  if (Platform.isOhos) {
    final bytes = await OhosFilePicker.pickFile();
    ...
    try {
      ...
      debugPrint('PILI_IMPORT_ERR ohosFile: $e\n$s');
      SmartDialog.showToast('导入失败：$e');
    }
    return;
  }
  ...
  if (result != null) {
    ...
    try {
      ...
      debugPrint('PILI_IMPORT_ERR localFile: $e\n$s');
      SmartDialog.showToast('导入失败：$e');
    }
  }
}
          ...
          onPressed: () async {
            if (key.currentState?.validate() == true) {
              try {
                ...
                debugPrint('PILI_IMPORT_ERR input: $e\n$s');
